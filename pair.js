...
case 'cp': {
    ...
    await socket.sendMessage(sender, { 
        text: `*❪ SEARCHING ❫*\n\n🔍 *Searching Chithrapata.com for:* _${query}_\n⚡ _Please wait a moment..._`
    }, { quoted: msg });

    try {
        const res = await axios.get(`${API_BASE}/api/v1/chithrapata/search?q=${encodeURIComponent(query)}&api_key=${API_KEY}`);
        const results = res.data.data || res.data.results || [];

        if (!results.length) {
            await socket.sendMessage(sender, {
                text: `*❪ NO RESULTS ❫*\n\n😞 *No Results Found on Chithrapata.com!*\n🎬 *Query:* _${query}_${DEFAULT_FOOTER}`
            }, { quoted: msg });
            break;
        }

        let listText = `*❪ CHITHRAPATA.COM SEARCH RESULTS ❫*\n\n🎯 *Query:* _${query}_\n📊 *Total:* _${results.length} Items_\n\n*👇 SELECT A NUMBER 👇*\n\n`;
        ...
        const handleSelection = async ({ messages: replyMessages }) => {
            ...
            if (isReplyToSentMsg && sender === replyMek.key.remoteJid) {
                ...
                if (isNaN(choice) || choice < 0 || choice >= results.length) {
                    await socket.sendMessage(sender, {
                        text: `⚠️ *Invalid choice! Range: 01 - ${results.length}*`
                    }, { quoted: replyMek });
                    return;
                }
                ...
                // MOVIE FLOW
                await socket.sendMessage(sender, { 
                    text: `*❪ FETCHING ❫*\n\n🎬 *Fetching Movie details from Chithrapata.com...*\n⚡ _Please wait..._`
                }, { quoted: replyMek });

                try {
                    const detailsRes = await axios.get(`${API_BASE}/api/v1/chithrapata/infodl?q=${encodeURIComponent(selectedItem.link || selectedItem.url)}&api_key=${API_KEY}`);
                    ...
                    const handleDownloadSelection = async ({ messages: dlReplyMessages }) => {
                        ...
                        if (isReplyToDlMsg && sender === dlReplyMek.key.remoteJid) {
                            ...
                            if (isNaN(dlChoice) || dlChoice < 0 || dlChoice >= validDownloads.length) {
                                await socket.sendMessage(sender, { text: `⚠️ *Invalid quality number!*` }, { quoted: dlReplyMek });
                                return;
                            }
                            ...
                            try {
                                ...
                                await socket.sendMessage(sender, {
                                    text: `⚠️ *Direct upload error:* ${sendErr.message}\n\n🔗 *Direct Link:* ${fileUrl}${DEFAULT_FOOTER}`
                                }, { quoted: dlReplyMek });
                            }
                    ...
                    await socket.sendMessage(sender, { text: `❌ *Details Error:* ${detailsErr.message}${DEFAULT_FOOTER}` }, { quoted: replyMek });
                }
...
case 'mslk': {
    ...
    await socket.sendMessage(sender, { 
        text: `*❪ SEARCHING ❫*\n\n🔍 *Searching MovieSubLK for:* _${query}_\n⚡ _Please wait a moment..._`
    }, { quoted: msg });

    try {
        const res = await axios.get(`${API_BASE}/api/v1/movie/moviesublk/search?q=${encodeURIComponent(query)}&api_key=${API_KEY}`);
        const results = res.data.data || res.data.results || [];

        if (!results.length) {
            await socket.sendMessage(sender, {
                text: `*❪ NO RESULTS ❫*\n\n😞 *No Results Found on MovieSubLK!*\n🎬 *Query:* _${query}_${DEFAULT_FOOTER}`
            }, { quoted: msg });
            break;
        }
        ...
        const handleSelection = async ({ messages: replyMessages }) => {
            ...
            if (isReplyToSentMsg && sender === replyMek.key.remoteJid) {
                ...
                // MOVIE FLOW
                await socket.sendMessage(sender, { 
                    text: `*❪ FETCHING ❫*\n\n🎬 *Fetching Movie details from MovieSubLK...*\n⚡ _Please wait..._`
                }, { quoted: replyMek });

                try {
                    const detailsRes = await axios.get(`${API_BASE}/api/v1/movie/moviesublk/infodl?q=${encodeURIComponent(selectedItem.link || selectedItem.url)}&api_key=${API_KEY}`);
                    const detailsData = detailsRes.data;
                    const movieInfo = detailsData.data || {};
                    const validDownloads = movieInfo.downloads || [];
...
case 'ss': {
    ...
    await socket.sendMessage(sender, { 
        text: `*❪ SEARCHING ❫*\n\n🔍 *Searching SinhalaSub.lk for:* _${query}_\n⚡ _Please wait a moment..._`
    }, { quoted: msg });

    try {
        const res = await axios.get(`${API_BASE}/api/v1/movie/sinhalasub/search?q=${encodeURIComponent(query)}&api_key=${API_KEY}`);
        const results = res.data.data || res.data.results || [];

        if (!results.length) {
            await socket.sendMessage(sender, {
                text: `*❪ NO RESULTS ❫*\n\n😞 *No Results Found on SinhalaSub.lk!*\n🎬 *Query:* _${query}_${DEFAULT_FOOTER}`
            }, { quoted: msg });
            break;
        }
        ...
        const handleSelection = async ({ messages: replyMessages }) => {
            ...
            if (isReplyToSentMsg && sender === replyMek.key.remoteJid) {
                ...
                if (isTvShow) {
                    await socket.sendMessage(sender, { 
                        text: `*❪ FETCHING ❫*\n\n📺 *Fetching TV Series details from SINHALASUB.LK...*\n⚡ _Please wait..._`
                    }, { quoted: replyMek });

                    try {
                        const tvRes = await axios.get(`${API_BASE}/api/v1/movie/sinhalasub/infodl?q=${encodeURIComponent(selectedItem.link || selectedItem.url)}&api_key=${API_KEY}`);
                        ...
                        await socket.sendMessage(sender, { text: `❌ *TV Show Error:* ${tvErr.message}${DEFAULT_FOOTER}` }, { quoted: replyMek });
                    }
                    socket.ev.off('messages.upsert', handleSelection);
                    return;
                }
                ...
                // MOVIE FLOW
                await socket.sendMessage(sender, { 
                    text: `*❪ FETCHING ❫*\n\n🎬 *Fetching Movie details from SinhalaSub.lk...*\n⚡ _Please wait..._`
                }, { quoted: replyMek });

                try {
                    const detailsRes = await axios.get(`${API_BASE}/api/v1/movie/sinhalasub/infodl?q=${encodeURIComponent(selectedItem.link || selectedItem.url)}&api_key=${API_KEY}`);
                    const detailsData = detailsRes.data;
                    const movieInfo = detailsData.data || {};
                    const validDownloads = movieInfo.downloads || [];
